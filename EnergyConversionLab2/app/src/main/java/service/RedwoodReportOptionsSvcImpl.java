...
public class RedwoodReportOptionsSvcImpl implements IOption
{
    ...
    private Bitmap kin, dyn;
    private List<String> kinlist, temlist, oillist, dynlist, timelist, weight1list, weight2list;
    private Boolean isSaybolt;

    public RedwoodReportOptionsSvcImpl(Activity activity, Bitmap kin, Bitmap dyn)
    {
    ...
    public List<String> loadRowList()
    {
        isSaybolt = bundle.getBoolean("isSaybolt");
        kinlist = bundle.getStringArrayList("kinematicViscosityList");
        dynlist = bundle.getStringArrayList("dynamicViscosityList");
        oillist = bundle.getStringArrayList("oilDensity");
        temlist = bundle.getStringArrayList("temperature");
        timelist = bundle.getStringArrayList("time");
        weight1list = bundle.getStringArrayList("flask_oil_weight");
        weight2list = bundle.getStringArrayList("flask_oil_weight");

        int size = kinlist.size();

        List<String> row1 = new ArrayList<>();
        if(isSaybolt)
            row1.add("Experiment - Saybolt Viscometer" + "\n");
        else
            row1.add("Experiment - Redwood Viscometer" + "\n");
        row1.add("Readings -");
        row1.add("Temperatures at which the behavior is observed = ");
        loadList(row1, temlist, size);
        row1.add("Time for collecting given volume of fluid = ");
        loadList(row1, timelist, size);
        row1.add("Weight of measuring jar = ");
        loadList(row1, weight1list, size);
        row1.add("Weight of measuring jar and the fluid = ");
        loadList(row1, weight2list, size);
        row1.add("\n" + "Result -");
        row1.add("Kinamatic viscosity of the fluid = ");
        loadList(row1, kinlist, size);
        row1.add("Density of oil = ");
        loadList(row1, oillist, size);
        row1.add("Dynamic viscosity of the fluid = ");
        loadList(row1, dynlist, size);
        row1.add("-----------------------------------------------------");

        return row1;
    }

    public void loadList(List<String> rows, List<String> pickFrom, int size)
    {
        for(int i = 0; i < size; i++)
        {
            rows.add(pickFrom.get(i));
        }
    }
    ...
    public void shareFile ( File created, String img1, String img2 )
    {
        ...
        if(created != null || created.exists()){
            ...
            String[] attachPath = new String[] {"/sdcard/EnergyConversionLab/" + img1, "/sdcard/EnergyConversionLab/" + img2, created.getAbsolutePath()};
            for (String file : attachPath) {
                File fileIn = new File(file);
                Uri url = Uri.fromFile(fileIn);
                uris.add(url);
            }
                ...
                layout_options_new_b.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        ...
                        if (new_reportName.isEmpty() || new_reportName.equals(null)) {
                            ...
                            if(!reportNames.contains(new_reportName + ".pdf"))
                            {

                                reportNames.add(new_reportName + ".txt");
                                fileIO.write(masterFile, reportNames);
                                createFile(new_reportName + ".txt");
                                saveImage( new_reportName + "_kinamaticV.png", new_reportName + "_dynamicV.png" );
            ...
            public void onClick(View v) {
                ...
                layout_options_existing_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id)
                    {
                        ...
                        String subTempName = tempName.substring(0, tempName.length() - 4);
                        addToExistingFile(tempName);
                        saveImage(subTempName + "_kinamaticV.png", subTempName + "_dynamicV.png");
                ...
                layout_options_delete_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ...
                        if (fileIO.deleteFile(tempName)) {
                            fileIO.deleteFile(subTempName + "_kinamaticV.png");
                            fileIO.deleteFile(subTempName + "_dynamicV.png");
                            Toast.makeText(activity, "File deleted....",
                                    Toast.LENGTH_LONG).show();
                        } else {
                            fileIO.deleteFile(tempName + "_kinamaticV.png");
                            fileIO.deleteFile(tempName + "_dynamicV.png");
            ...
            public void onClick(View v) {
                ...
                layout_options_share_list.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                        ...
                        saveImage(tempName + "_kinamaticV.png", tempName + "_dynamicV.png");
                        shareFile(tempFile, tempName + "_kinamaticV.png", tempName + "_dynamicV.png" );
    ...
    public void saveImage(String kinName, String dynName)
    {
        FileOutputStream outKin = null;
        FileOutputStream outDyn = null;
        try {
            outKin = new FileOutputStream("/sdcard/EnergyConversionLab/" + kinName);
            outDyn = new FileOutputStream("/sdcard/EnergyConversionLab/" + dynName);
            kin.compress(Bitmap.CompressFormat.PNG, 90, outKin);
            dyn.compress(Bitmap.CompressFormat.PNG, 90, outDyn);
